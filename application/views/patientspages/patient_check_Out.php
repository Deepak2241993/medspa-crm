...
<?=form_open('patients/save_patients_checkout')?>
   <!--  <h5> </h5>  -->
            ...
            <select name="pname" class="form-control"  id="pname">
              ...
              <?php if(!empty($patients_list)){
                    foreach($patients_list as $patients){?>
                        <option value="<?= $patients->pid?>" <?php if(isset($patients_note_data[0]['pid'])) echo ($patients->pid ==  $patients_note_data[0]['pid']) ? ' selected="selected"' : '';?>><?= $patients->pname?></option>
                    <?php }} ?>
            </select>
        </div>

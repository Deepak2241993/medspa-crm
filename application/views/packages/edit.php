...
<div class="main-content">
	...
	<div class="card">
		<div class="card-body">
			<h4>Add Package</h4>
			<div class="m-t-25">
				<div class="form-row dataappended">
		...
		<div class="card" style="display:block;" id="service_assign">
			<div class="card-body">
				<h4>Select Service For Package Build</h4>
					<?php
						$service_cat_id=json_decode($packages[0]->service_cat_id);
						$service_id=json_decode($packages[0]->service_id);
						// print_r($service_cat_id); die();
						$service_session=json_decode($packages[0]->service_session);
						
						if (is_array($service_cat_id)) {

						foreach($service_cat_id as $keyy=>$pvalue)
						{
							$serviceCategory = $this->MasterModel->get_databy_id($pvalue,'service_category');
							// print_r($serviceCategory); die();
							$n_services = $this->MasterModel->get_databy_id($service_id[$keyy],'services');
						?>
				<div class="form-row">
						  
						
					<div class="form-group col-md-3">
						<label for="service">Category Name<span class="text-danger">*</span><?=$pvalue?></label>
						
						<select class="form-control" name="service_cat_id[]" id="category" required>
							<option value="<?=$serviceCategory[0]->id?>"><?=$serviceCategory[0]->name?></option>
						</select>
					</div>
					<div class="form-group col-md-3" id="service_remove_<?=$keyy?>">
						<label for="name">Service Name<span class="text-danger">*</span></label>
						<select class="form-control" name="service_id[]" id="category" required>
							<option value="<?=$n_services[0]->id?>"><?=$n_services[0]->service_name?></option>
						</select>
						
					</div>
					<div class="form-group col-md-2" id="service_session<?=$keyy?>">
						<label for="service_session<?=$keyy?>">Service Session<span class="text-danger">*</span></label>
						<select class="form-control"name="service_session[]" required>
							<option value="<?=isset($packages[0]->service_session)?$service_session[$keyy]:''?>"><?=isset($packages[0]->service_session)?$service_session[$keyy]:''?></option>
						</select>
						
					</div>
					<div class="form-group col-md-2">
						<label for="amount<?=$keyy?>">Amount</label>
						<input type="text"class="form-control"  name="amount[]" value="<?=isset($packages[0]->price)?($n_services[0]->service_charge)*$service_session[$keyy]:''?>">
						
						
					</div>
					<div class="form-group col-md-2 mt-4" id="service_button_<?=$keyy?>">
...
<script>
	...
	$.ajax({
	 ...
	 function servicePriceShow(element, sectionCount) {
		 var get_service_id = $(element).val();
